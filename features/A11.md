...
| I2C              | 5.16 / 6.18 (dts)               |
| SPMI             | 6.18                            |
| Battery Info     | linux-apple                     |
| Power Down       | linux-apple[2]                  |
| Display Pipe     | TBA                             |
| CPU PMU          | linux-apple                     |
...
especially when booting complex systemd-based GUI environments.
On resetting the NVME controller (`echo 1 > /sys/class/nvme/nvme0/reset_controller`),
a crash also happens with the same message. It appears that the
error occured while setting up the admin queue.
(HELP WANTED!)

- [2]: PRC shutdown behavior implemented, but not tested on PRC device.
(Tested via spoofing ADT /region-info on other devices)
